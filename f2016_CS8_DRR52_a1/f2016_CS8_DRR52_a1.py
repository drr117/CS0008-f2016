...
if (units == "USC"): #setup a conditional statement if the user selects USC
    ...
    if (rating > 20): #setup an if statement for the fuel rating; following the ranges from the table; do this using if and elif statements
        fuel_rating = ("Extremely Poor") #This variable and the following elif statements follow the table values for fuel rating
    elif (rating > 15 and rating <= 20): #and conditional is used for range
        fuel_rating = ("Poor")
    ...
    print 'Distance:', format(distance, '10.3f'), 'miles', format(the_distance_convert, '10.3f'), 'km'  #the print function is used at the end with the format to neatly display the results of the users input
    print "Gas:", format(fuel, '13.3f'), 'gallons', format(the_fuel_convert, '13.3f'), 'liters' #increased the spacing for fuel because it was too close to other values
    print "Consumption:", format(d, '10.3f'), 'mpg', format(the_fuel_consumption_convert, '10.3f'), 'L/100km'
    print "gas consumption rating:", fuel_rating #this prints at the end


else:
    if (units == "METRIC"): #repeats the same process as before only with a different value
        ...
        the_fuel_consumption_convert = (the_distance_convert / the_fuel_convert) #converted differently because it is simpler to use variables as opposed to numbers
    ...
    #essentially the last part repeats the first part (USC is the first part)
